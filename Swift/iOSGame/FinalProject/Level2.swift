...
class Level2: Level
{
    ...
    {
        _time = 0
        _nextTimes = [LevelTimes(timing: 3.0, type: enemyDict),
                      LevelTimes(timing: 6.0, type: enemyDict),
                      LevelTimes(timing: 14.0, type: enemyDict),
                      LevelTimes(timing: 15.0, type: asteroidDict),
    ...
    private func generatePath(number: Int, type: String) -> [Path]
    {
        ...
        case 0:
            ...
            break
        case 1:
            paths.append(Path(x: 0.0, y: -0.1, fire: false))
            paths.append(Path(x: 0.0, y: 1.5, fire: true))
            break
        case 2:
            paths.append(Path(x: 0.3, y: 0.5, fire: false))
            paths.append(Path(x: -0.8, y: 0.5, fire: true))
            break
        case 3:
            paths.append(Path(x: paths.first!._x, y: -2.0, fire: true))
            break
        case 4:
